...
export default function QuizModal({ quiz, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6"
      ...
      <div className="quiz-pop w-full max-w-lg bg-white rounded-3xl p-7 shadow-2xl border border-white/80 max-h-[90vh] overflow-auto"
        ...
        {answered && (
          <div className={`rounded-2xl p-4 mb-4 text-sm font-semibold ${correct ? "bg-green-50 text-green-800 border border-green-200" : "bg-amber-50 text-amber-800 border border-amber-200"}`}>
            ...
            {correct && quiz.badge && (
              <span className="ml-2 inline-block px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-xs font-black">
                {quiz.badge}
              </span>
            )}
          </div>
