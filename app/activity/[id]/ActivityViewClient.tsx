  ...
  if (steps.length === 0) {
    return (
      <div style={{ minHeight: "100vh", background: "#F8F8F6", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
        <AppNav activePage="apply" userName={profile?.full_name} isAdmin={profile?.role === "admin" || profile?.role === "superadmin"} />
            ...
            <>
              ...
              <div className={s.belowScreen}>
                <div className={s.askInline}>
                  ...
                  <input
                    className={s.askInlineInput}
                    value={input}
                    onChange={e => setInput(e.target.value)}
                    onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) sendInline(); }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {content!.downloads.map((d, i) => (
                  <a key={i} href={d.url} download target="_blank" rel="noreferrer"
                    style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 10, border: "1px solid #e4eaf2", background: "#fff", textDecoration: "none", color: "#101828", fontSize: 13, fontWeight: 700 }}>
                    <span style={{ fontSize: 16 }}>{dlIcon(d.type)}</span>
                ...
                {content!.prompts.map((p, i) => (
                  <div key={i} style={{ border: "1px solid #e4eaf2", borderRadius: 10, overflow: "hidden" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 12px", background: "#f9fbfd", borderBottom: "1px solid #e4eaf2" }}>
                      ...
                      <button type="button" onClick={() => { navigator.clipboard.writeText(p.text).then(() => { setCopiedIdx(i); setTimeout(() => setCopiedIdx(null), 2000); }); }}
                        suppressHydrationWarning
                        style={{ border: "1px solid", borderColor: copiedIdx === i ? "rgba(35,206,104,.3)" : "#e4eaf2", background: copiedIdx === i ? "rgba(35,206,104,.08)" : "white", color: copiedIdx === i ? "#16A34A" : "#667085", borderRadius: 999, padding: "3px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
      ...
      <aside className={`${s.drawer} ${activeDrawer === "chat" ? s.drawerOpen : ""}`}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "#94A3B8", fontWeight: 500 }}>
                {isOverview ? "ready when you are 👋" : `step ${current + 1} · ${step?.title ?? ""}`}
              </div>
            </div>
          </div>
          <button type="button" className={s.closeBtn} onClick={() => setActiveDrawer(null)}>×</button>
        ...
        <div className={s.chatDrawerBody}>
          <div ref={chatRef} className={s.chatMessages}>
            {initializing && (
              <div className={`${s.bubble} ${s.bubbleAi}`} style={{ color: "#94A3B8" }}>Preparing your session…</div>
            )}
            {messages.map((m, i) => (
            ...
            {loading && (
              <div className={`${s.bubble} ${s.bubbleAi}`} style={{ color: "#94A3B8" }}>Thinking…</div>
            )}
          </div>
          ...
          {!chipsDismissed && currentChips.length > 0 && !loading && (
            <div className={s.chatChips}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 10.5, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".08em", color: "#94A3B8" }}>
              ...
              {currentChips.map((chip, i) => (
                <button key={i} type="button" className={s.chip}
